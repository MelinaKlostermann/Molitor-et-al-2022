...
output <- "/Users/melinaklostermann/Documents/projects/PURA/Molitor-et-al-2022/"
``` 
...
ggsave(paste0(output, "acessibility.pdf"), width = 6, height = 6, units = "cm")
...
plot_freq_scatter <- function(gr_regions, bg, oligo, title, window, color_thresh, filename){
  ...
  ggsave(filename = filename,  width = 12, height=3, device = "pdf", path = output)
...
plot_freq_scatter(gr_regions = c(BS_CDS, BS_3UTR)+5, bg = c(random_bg_cds, random_bg_3utr)+5, oligo = 5, window =20, color_thresh = 3,
...
# Sequence logos
- Here are the seqeuncing logos for 3'UTR and CDS binding sites and their surrounding
```{r logos}

 BS_3UTR_10nt <- BS_3UTR + 10
 BS_3UTR_10nt <- BS_3UTR_10nt[BS_3UTR_10nt %within% transcriptome_seqs_coords]
 BS_3UTR_10nt <- BS_3UTR_10nt[width(BS_3UTR_10nt)==25]
 
 BS_CDS_10nt <- BS_CDS + 10
 BS_CDS_10nt <- BS_CDS_10nt[BS_CDS_10nt %within% transcriptome_seqs_coords]
 BS_CDS_10nt <- BS_CDS_10nt[width(BS_CDS_10nt)==25]


 # make logo both
 BS_25nt_window_seq <-  getSeq(transcript_fasta, names=c(BS_CDS_10nt, BS_3UTR_10nt)) %>%
   as.data.frame()

  rect_BS <- data.frame(xmin=10.5, xmax=15.5, ymin=-Inf, ymax=Inf)
  
 ggplot()+
   geom_logo(BS_25nt_window_seq$x)+ #, method = "probability"
   theme_logo()+
     geom_rect(data=rect_BS, inherit.aes=FALSE, aes(xmin=xmin, xmax=xmax, ymin=ymin,
                 ymax=ymax), color="darkred", fill="transparent", alpha=0.3)+
   ggtitle("Sequence Logo around CDS and 3'UTR BS")
 ggsave(paste0(output,"logo_cds_and_3utr_BS.pdf"), width = 5, height = 4, units = "cm")
 
```

## Purine probability in binding sites

```{r}
# How many Gs at center?
center_nt <- table(substring(BS_25nt_window_seq$x,13,13))

center_nt["G"]/sum(center_nt)


# More then x purines
BS_seq_df <- rbind(as.data.frame(seqences_BS_3utr), as.data.frame(seqences_BS_cds))
BG_utr_seq_df <- as.data.frame(seqences_bg_3utr) 
BG_cds_seq_df <- as.data.frame(seqences_bg_cds)



nts_per_BS_df <- mutate(BS_seq_df, n_purines = stringr::str_count(x, "A") + stringr::str_count(x, "G"))
nts_per_utr_BG_df <- mutate(BG_utr_seq_df, n_purines = stringr::str_count(x, "A") + stringr::str_count(x, "G"))
nts_per_cds_BG_df <- mutate(BG_cds_seq_df, n_purines = stringr::str_count(x, "A") + stringr::str_count(x, "G"))


purines_per_BS <- data.frame(n_purines = 0:5, n_BS = rep(NA,6), n_utr_BG = rep(NA,6), n_cds_BG = rep(NA,6)) 

for( i in 0:5){
purines_per_BS$n_BS[i+1] <- nrow(nts_per_BS_df[nts_per_BS_df$n_purines >= i,])/ nrow(nts_per_BS_df)
purines_per_BS$n_utr_BG[i+1] <- nrow(nts_per_utr_BG_df[nts_per_utr_BG_df$n_purines >= i,])/ nrow(nts_per_utr_BG_df)
purines_per_BS$n_cds_BG[i+1] <- nrow(nts_per_cds_BG_df[nts_per_cds_BG_df$n_purines >= i,])/ nrow(nts_per_cds_BG_df)
}

purines_per_BS_gg <- reshape2::melt(purines_per_BS, id.vars = "n_purines")
purines_per_BS_gg$round <- round(purines_per_BS_gg$value, 2)

ggplot(purines_per_BS_gg, aes(x = n_purines, y = value, fill = variable, label = round))+
  geom_col(position = "dodge")+
  scale_fill_manual(values = c("darkred",  "darkgrey", "grey"))+
  theme_paper()+
  geom_text()+
  theme(legend.position = NULL)

ggsave(paste0(output, "purnies_per_bs.pdf"), width = 6, height = 5)
```
