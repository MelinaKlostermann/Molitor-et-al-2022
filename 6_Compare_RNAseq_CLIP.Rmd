...
# boxplot regions 
plot_box_region <- ggplot(res_RNAseq_CLIP[(res_RNAseq_CLIP$padj > 0.01),], aes(x=region, y=log2FoldChange, fill = region))+
  geom_boxplot()+
  theme_paper()+
...
ggplot()+
  ggrastr::rasterise(geom_point(data = res_RNAseq_CLIP[res_RNAseq_CLIP$sig.change ==F,],  aes(x=log2FoldChange, y= -log10(padj)), color = "grey", shape = 1,  size = 0.5), dpi = 300)+
  ggrastr::rasterise(geom_point(data = res_RNAseq_CLIP[res_RNAseq_CLIP$sig.change ==T,], aes(x=log2FoldChange, y= -log10(padj)), size = 0.5), dpi = 300)+
  ggrastr::rasterise(geom_point(data = res_RNAseq_CLIP[res_RNAseq_CLIP$sig.change ==T & res_RNAseq_CLIP$region =="no_region",], aes(x=log2FoldChange, y= -log10(padj)), color = "purple",  size = 0.5), dpi = 300)+
  theme_bw()+
  xlim(c(-2,2))+
  ylim(c(0,40))+
  theme_paper()
