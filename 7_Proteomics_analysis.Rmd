...
- there are two different reasons for this: 
 ...
 2. multiple genes are grouped togeter because of their sequence similarity
...
info_biomart_new<- getBM(attributes=my_attributes,
                        filters = "hgnc_symbol",
                        values = maxquant_table$gene_name,
                        mart = ensembl38p13, useCache = F)

#combine
idx_biomart_new <- match(maxquant_table$gene_name, info_biomart_new$hgnc_symbol)
...
r_down = hyps_reactome[[2]]$data %>% mutate(group = "down", padj = p.adjust(pval, method = "BH"))
r_up = hyps_reactome[[1]]$data %>% mutate(group = "up",  padj = p.adjust(pval, method = "BH"))
...
ggplot(reactome_go_cust, aes(y = generatio, x = factor(label, level = rev(unique(label))), fill = padj <0.05))+
  geom_col()+
  coord_flip()+
  theme_paper()+
...
cc_down = hyps_cell_comp[[2]]$data %>% mutate(group = "down", padj = p.adjust(pval, method = "BH"))
cc_up = hyps_cell_comp[[1]]$data %>% mutate(group = "up",  padj = p.adjust(pval, method = "BH"))
...
ggplot(cc_go_cust, aes(y = generatio, x = factor(label, level = rev(unique(label))), fill = padj <0.01))+
  geom_col()+
  coord_flip()+
  theme_paper()+
  scale_fill_manual(values = setNames(c('blue','darkgrey'),c(T, F)))+
...
########################
gois = c("CTNNA1", "SQSTM1", "LSM14A", "CUX1", "DDX6", "GAPDH", "PURA", "PURB", "STARD7", "IL6ST", "DCP1A")
gg_df <- prot_vs_RNA %>%
  rowwise() %>%
...
prot_vs_RNA_BS <- left_join(prot_vs_RNA, BS_strongest_per_gene[c("seqnames", "start", "end", "width", "strand", "gene_id", "score", "region")], by =
                              c(gene_id.rnaseq= "gene_id")) %>% ungroup(.) %>% as.data.frame(.)

prot_vs_RNA_BS_gg <- prot_vs_RNA_BS %>% mutate(., bound = case_when( !is.na(region) ~ "PURA-bound",
                                                                  T ~ "Not bound")) 


# color vulcano by regions
ggplot(prot_vs_RNA_BS_gg, aes(y = -log10(sca.adj.pval.prot), x = logFC.prot))+
  geom_point(data=dplyr::select(prot_vs_RNA_BS_gg, -region), aes(y = -log10(sca.adj.pval.prot), x = logFC.prot))+
