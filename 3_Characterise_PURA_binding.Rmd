...
```{r}
## TODO move to 0 annotation filter
...
ol <- findOverlaps(annotation_genes, binding_sites, ignore.strand=F)
bs <- binding_sites
bs$idx <- 1:NROW(bs)

# get all possible regions overlapping with crosslinked nucleotide
bs_2 <- bs[subjectHits(ol)]
elementMetadata(bs_2) <-  c(elementMetadata(bs_2), elementMetadata(annotation_genes[queryHits(ol), c("gene_id", "gene_type", "gene_name")]))

table(bs_2$gene_type)

# chose for each binding site a gene from the highest possible hierarchy
# if to genes from the same type would be present the first one is chosen
bs_3 <- bs_2 %>%  as.data.frame(.) %>%
  group_by(idx) %>%
  arrange(factor(gene_type, levels = c("lncRNA", "miRNA", "miscRNA", "snRNA", "snoRNA", "protein_coding" )), .by_group = T) %>% 
  dplyr::slice(1) 

binding_sites <- makeGRangesFromDataFrame(bs_3, keep.extra.columns = T)
bound_genes <- bs_3[!duplicated(bs_3$gene_id),]
...
bs_pc <- binding_sites[binding_sites$gene_type=="protein_coding"]
ol_bs <- findOverlaps(anno_regions, bs_pc-2, ignore.strand = FALSE)

bs_pc$idx <- 1:NROW(bs_pc)

# get all possible regions overlapping with crosslinked nucleotide
bs_2 <- bs_pc[subjectHits(ol_bs)]
anno_bs <- elementMetadata(anno_regions[queryHits(ol_bs)]) %>% as.data.frame()
colnames(anno_bs) <- paste0(colnames(anno_bs), ".from_region")
elementMetadata(bs_2) <- c(elementMetadata(bs_pc[subjectHits(ol_bs)]), anno_bs)


# choose type highest in hiearcy
bs_3 <- as.data.frame(bs_2) %>%
  filter(., gene_id == gene_id.from_region) %>% # this throws out regions that overlap with non-coding RNAs, with this line no binding sites in introns!!!
  group_by(idx) %>%
  arrange(factor(type.from_region, levels = c("three_prime_UTR", "five_prime_UTR", "CDS", "intron")), .by_group = T) %>%
  dplyr::slice(1) # arrange by non-coding type


bs3 <- makeGRangesFromDataFrame(bs_3, keep.extra.columns = T)
bs_without_region <- binding_sites[-queryHits(findOverlaps(binding_sites, bs3))]
bs_without_region$type.from_region <- "no_region"

# plot for paper
ggplot(bs_3, aes(x = type.from_region ))+
  geom_bar()+
...
```{r}
# add protein_coding Bs with region to non-coding BS without region
binding_sites_with_regions <- c(makeGRangesFromDataFrame(bs_3, keep.extra.columns = T) ,
                                bs_without_region) %>% 
  as.data.frame(.)

# final numbers
table(binding_sites_with_regions$type.from_region)
table(binding_sites_with_regions[!duplicated(binding_sites_with_regions$gene_id),]$gene_type)

# clean up unnecessary columns
binding_sites_with_regions <- binding_sites_with_regions[,c(1:15, 17)]
colnames(binding_sites_with_regions) <- c(colnames(binding_sites_with_regions)[1:15], "region")
...
norm_regions <- bs_3 %>% group_by(type.from_region) %>% summarize(nts_in_BS = sum(width)) %>% 
  left_join(norm_df, by = c(type.from_region = "region")) %>%
  mutate(norm_binding_per_reg_size = nts_in_BS / width)



ggplot(norm_regions, aes(x=type.from_region, y = norm_binding_per_reg_size))+
  geom_col()+
  theme_paper()
...
## TODO this file changed now, rerun all following skripts with new file
