...
annotation_genes <- annotation[annotation$type=="gene"] # use transcripts instead

#overlaps of bs with genes
ol <- findOverlaps(annotation_genes, binding_sites, ignore.strand=F)
binding_sites[-subjectHits(findOverlaps(annotation_genes, binding_sites, ignore.strand=F))]
...
elementMetadata(binding_sites_temp1) <-  c(elementMetadata(binding_sites_temp1), elementMetadata(annotation_genes[queryHits(ol), c("gene_id", "gene_type", "gene_name")]))
...
anno_bs <- elementMetadata(anno_regions[queryHits(ol_bs)]) %>%
  as.data.frame(.) %>%
  dplyr::select(c("type", "gene_id", "transcript_id"))

# and add to temp binding sites
colnames(anno_bs) <- c("type", "gene_id.from_region", "transcript_id.from_region")
...
table(binding_sites_with_regions$type)

# why do some many protein coding genes not have a region? (~3500) --> all with region when annotation is not filtered
# 
bs_without_region <- binding_sites[-queryHits(findOverlaps(binding_sites, binding_sites_with_regions))]

anno_regions[queryHits(findOverlaps(anno_regions, bs_without_region , ignore.strand = FALSE))]$gene_type


table(bs_without_region$region)

#binding_sites <- c(binding_sites_with_region , bs_without_region )
...
colnames(elementMetadata(binding_sites_with_regions)) <- c(colnames(elementMetadata(binding_sites_with_regions))[1:10], "region", "", "transcript_id")
elementMetadata(binding_sites_with_regions) <- elementMetadata(binding_sites_with_regions)[c(1:11,13)]
