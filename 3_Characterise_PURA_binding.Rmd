...
table(bs_2[duplicated(bs_2)]$gene_type)


bs_3 <- bs_2 %>%  as.data.frame(.)  %>%
  filter( gene_type != "TEC" & !grepl(pattern = "pseudo", gene_type)  ) %>%
  makeGRangesFromDataFrame(keep.extra.columns = T)# remove uninteresting types

NROW(bs_3)

table(bs_3[duplicated(bs_3)]$gene_type)
...
- gene regions are assignes vis hieracry: "three_prime_UTR", "five_prime_UTR", "CDS", "intron"
- Problem: binding sites seem in IGV to be localised rather in non-coding RNAs that overlap introns then in introns (crosslinks only inside non-coding RNA) --> show be assigned rather to the non-coding RNA then to the preotein coding gene of the intron

Strategy for binding site characterisation:
- binding sites on protein coding genes are directly assigned to their region by hieracrcy: "three_prime_UTR", "five_prime_UTR", "CDS", "non-coding", "intron"
- problem: lncRNAs have 3'UTRs etc assigned, miRNAs are only assigned as gene
- in region plot we only show regions of protein coding genes anyway
- therefore all non-coding RNAs are assigned to "region" non-coding irrespective of where in the non-coding RNA the binding site lyes
...
# get all non coding RNAs
non_coding_gene_annotation <- annotation %>% as.data.frame(.) %>%
  filter(gene_type != "protein_coding", type == "gene") %>%
  mutate(., type =  "non_coding_gene_without_region" ) %>% makeGRangesFromDataFrame(keep.extra.columns = T)

table(non_coding_gene_annotation$gene_type )

#get introns
introns <- intronsByTranscript(anno_txdb, use.names=T) %>% unlist(.)
introns$type = "intron"
anno_regions <- c(anno_regions, non_coding_gene_annotation, introns)

#overlaps of binding sites with genes
ol_bs <- findOverlaps(anno_regions, binding_sites-2, ignore.strand = FALSE)
bs <- binding_sites
bs$idx <- 1:NROW(binding_sites)
...
bs_3 <- as.data.frame(bs_2) %>%
  group_by(idx) %>%
  arrange(factor(type, levels = c("three_prime_UTR", "five_prime_UTR", "CDS", "intron", "non_coding_gene_without_region")), .by_group = T) %>%
  dplyr::slice(1)


table(bs_3$gene_type)
...
utr3 <- anno_for_norm[anno_for_norm$type=="three_prime_UTR"] %>% reduce()
anno_left <- GenomicRanges::setdiff(anno_for_norm_r, utr3, ignore.strand=FALSE)

utr5 <- GenomicRanges::intersect(anno_left, anno_for_norm[anno_for_norm$type=="five_prime_UTR"] )  %>% reduce()
anno_left <- GenomicRanges::setdiff(anno_left, utr5, ignore.strand=FALSE)

cds <- GenomicRanges::intersect(anno_left, anno_for_norm[anno_for_norm$type=="CDS"] )  %>% reduce()
