...
# Main objective: 
# - Overlap of PURA binding and dendritic expression and localization to p-bodies
#   and stress granules
# Input:
# - Binding information as RDS-File
# - DESeq2 results as RDS-File
# - TPM values as RDS-File
# - Dendritic expression, p-body and stress granule enrichment information as 
#   RDS-Files
#
# 
# Major/ issues:
# - None. Just picky minor comments :)
#
# Minor/ suggestions:
# - Sometimes a little bit more comments would be helpful
# - Most of the time you use the dplyr way to filter things but sometime you
#   switch to subsetting data.frames via brackets 
# - Sometimes you pipe the data.frame into dplyr functions and sometimes you 
#   use them as first argument in the function. Decide for one way.
# - Variable names are sometimes not very informative 
# - You forgot to multiply the bound ratio ($ratio_bound) by 100 in the plot
#
# Conclusion:
# - Analyses are reliable and trustworthy
# - Most of the time well commented
#
...
### TODO MK
### Above you claim to match by a 1-2-1 orthology but you removed this step.
### What is happening if the mouse gene matches more than one human gene?
### There are 103 duplication events where the mmusculus gene name occurs more than once:
###   info_biomart_new %>% filter(mmusculus_homolog_associated_gene_name %in% dendritic_RNAs$GeneName) %>% pull(mmusculus_homolog_associated_gene_name) %>% duplicated %>% sum
### I saw that you check for the 1-2-1 relation in the chunk below. I would then change the comment above.
...
### TODO MK
### I don't like the visualization as bar charts as it is quite misleading.
### From right to left the bars are subsets of the other bars. A venn diagram with 
### the eulerr package or a stacked bar chart wou would have been more appropriate
...
### TODO MK
### Probably a more elegant way if there are only two cases:
###   dendritic_RNAs <- mutate(dendritic_RNAs, bound = ifelse(ensembl_gene_id %in% BS$gene_id, "PURA-bound","No binding"))
...
### TODO MK
### The naming of your variables is sometimes confusing. Above you have a data.frame
### named hela_df and now you create df_hela. Sometimes a little bit more information
### in the variable names is helpful. It's not obivious that the data.frame stores
### dendritic genes with a 1-2-1 human gene that is expressed in HeLa cells
...
### TODO MK
### Sometimes you provide the data.frame as first argument in mutate() and 
### sometimes you pipe the data.frame into mutate() 
### Make it consistent.
...
### TODO MK
### In the rest of the chunk you always use [] for subsetting but you could have
### also used the dplyr-way:
###   tpms <- rnaseq_counts %>% dplyr::select(gene_id, mean_tpm_rnaseq)
###   pb_transcriptome_expr <- pb_transcriptome %>% filter(`Ensembl Gene ID` %in% tpms$gene_id)

### TODO MK
### Why do you have NAs in tpms$mean_tpm_rnaseq?
...
### TODO MK
### You need to multiply $ratio_bound by 100 to get percentage at a cutoff of 1000
### you have 100% bound and not 1%
